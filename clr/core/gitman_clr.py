...
def fuzzy_clr(X, y, k, kmeans_X=0.0, max_iter=5, verbose=0, lr=None):
    if lr is None:
        lr = Ridge(alpha=1e-5)
...
def clr(X, y, k, kmeans_X=0.0, constr=None, lr=None,
    ...
    weights = (labels == np.arange(k)[:, np.newaxis]).sum(axis=1).astype(float)
...
def best_clr(X, y, k, fuzzy=False, num_tries=10, **kwargs):
    ...
    best_obj = np.inf
    best_labels, best_models, best_weights = None, None, None
