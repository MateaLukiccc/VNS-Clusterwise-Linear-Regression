...
def _train_classification(X, a, K, random_state=42):
    """Random Forest classifier on (X, a). Returns a FRESH classifier every call."""
...
def _vnd(X, y, W_init, a_init, l_max, K, alpha, strategy, random_state):
    a_star = a_init.copy()
    W_star = W_init.copy()
    Theta_star = _train_classification(X, a_star, K, random_state) if alpha < 1.0 else None
    ...
    while l <= l_max:
        ...
        best = [err_star, a_star.copy(), W_star.copy()]
        try:
    ...
    Theta_star = _train_classification(X, a_star, K, random_state)
...
class CLR_VND:
    ...
    using a Random Forest classifier (instead of logistic regression) to assign
    new points to clusters at prediction time.
    """
    ...
    def __repr__(self):
        return ("CLR_VND_Tree(K={}, l_max={}, alpha={}, strategy={!r})"
...
if __name__ == "__main__":
    ...
    print("CLR_VND_Tree RMSE:", round(rmse(y, model.predict(X)), 4))
